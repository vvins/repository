...
SERVICE_NAME="elsalto-stp-sctp-association"
NSCA_IP1="172.16.37.143"
NSCA_IP2="172.16.37.144"
stp_pause="MTP-PAUSE: AffectedDpc=9844"
stp_resume="MTP-RESUME: AffectedDpc=9844"
...
function check_connection {
	...
	if [ "$pauseArrayLength" -eq 0 ]; then
		PLUGIN_OUTPUT="OK association between SCTP and Elsalto STP at $cur_min"
		RETURN_CODE=0
		send_notification
		...
		if [ "$resumeArrayLength" -eq 0 ]; then
			PLUGIN_OUTPUT="CRITICAL. ELSALTO-STP. $pauseArray[$lastelem] check $errorlog"
			...
			# echo "resumetime: $resumetime" 
				...
				if [ "$resumetime" -gt "$pausetime" ]; then                 # here we check hours,minutes and secs, if they equal check decimals of sec
						PLUGIN_OUTPUT="WARNING. ELSALTO-STP. MTP-PAUSE and MTP-RESUME in $logfile between $previous_min and $cur_min check $errorlog"
						RETURN_CODE=1
						send_notification
					...
					PLUGIN_OUTPUT="CRITICAL. $pauseArray[$lastelem] check $errorlog"
					RETURN_CODE=2
					send_notification
					echo "$pauseArray[$lastelem]" >> "$errorlog"
